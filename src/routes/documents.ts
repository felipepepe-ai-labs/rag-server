...
/** Chunked source entry for import results. */
export interface ImportSourceResult {
  filename: string;
  chunks: number;
}

/** Structured response envelope for bulk import. */
...
export async function readFileContents(req: import('node:http').IncomingMessage): Promise<Array<{ name: string; filename: string; content: string }>> {
  const chunks: Buffer[] = [];
  ...
  if (!boundaryMatch) throw new TypeError('Invalid Content-Type: no boundary found');
...
export function validateFiles(files: Array<{ filename: string }>): void {
  if (!files.length) throw new TypeError('At least one file is required');
  if (files.some(f => !f.filename)) throw new TypeError('All files must have a filename');
  if (files.length > FILE_MAX) throw new TypeError(`Too many files: max ${FILE_MAX} allowed`);
}

/** Chunk text into overlapping word-boundary segments. */
export function chunkByWords(text: string, size: number = CHUNK_SIZE, overlap: number = CHUNK_OVERLAP): string[] {
...
async function generateEmbedding(text: string): Promise<number[]> {
  ...
  if (!json.embeddings || !json.embeddings[0]) throw new Error('Empty embeddings from Ollama');

  return json.embeddings[0];
}

/** Factory that produces a /documents/import route handler. */
export function createImportRoute(vstore: VectorStore) {
  const handler = async (req: import('node:http').IncomingMessage): Promise<ImportResponse> => {
